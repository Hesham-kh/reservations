...
@method('patch')
  ...
  <div class="mb-3">
   ...
   <input type="namber" class="form-control" value="{{$product->oldPrice}}" name="oldPrice" id="price">
 </div>
  <div class="mb-3">
    <label for="pricenew" class="form-label">السعر الجديد</label>
    <input type="namber" class="form-control"  value="{{$product->newPrice}}" name="newPrice" id="pricenew">
  </div>
   <div class="mb-3">
  <label class="catagory" for="">الصنف</label>
  <select class="form-select" aria-label="Default select example" name="category">
  <option selected>اختر التصنيف</option>
  <option selected disabled value="{{$product->category_id}}">{{$product->category->name}}</option>
